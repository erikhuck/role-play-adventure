...
def login():
    if request.method == 'POST':
        if 'new_player' in request.form:
            ...
            if new_name not in db.players:
                ...
                return render_template('error.html', message=f'Player of name "{new_name}" already exists.')
            return redirect(url_for('auth.login'))
        elif 'player_name' in request.form:
            if 'player_name' in session:
                return render_template(
                    'error.html', message=f'You already logged in as "{session["player_name"]}".')
    ...
    if 'player_name' in session:
        return redirect(url_for('main.home'))
    return render_template('login.html', players=db.players.values())
